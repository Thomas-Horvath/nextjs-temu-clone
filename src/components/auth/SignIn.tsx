...
type SignInProps = {
  action: (prevState: unknown, formData: FormData) => Promise<{ message: string } | undefined>
}
...
const SignIn = ({ action }: SignInProps) => {
  ...
  return (
    <Form action={formAction} className='max-w-md mx-auto my-16 p-8 bg-white rounded-lg shadow-md'>
      ...
      <p className='text-center text-sm text-gray-600 mb-6'>Sign in to accsess your exclusive member deals.</p>
      ...
      <div className='space-y-6'>
        ...
        <button
          type='submit'
          disabled={isPending}
          className={`w-full bg-rose-600 text-white py-3 rounded-md hover:bg-rose-700 transition-colors font-medium flex items-center justify-center gap-2 ${isPending ? 'cursor-not-allowed' : ''} cursor-pointer`}
        >
          {isPending ? (
            <>
